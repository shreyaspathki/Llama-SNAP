...
export function SnapPopup() {
  ...
  return (
    <div className="w-[380px] h-[600px] bg-zinc-50 flex flex-col overflow-hidden shadow-2xl rounded-2xl border border-zinc-200 font-sans">
      {/* ── Header ─────────────────────────────────────────── */}
      <div className="bg-white border-b border-zinc-100 px-5 py-4 shrink-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 bg-emerald-500 rounded-xl flex items-center justify-center text-white font-black text-lg shadow-lg shadow-emerald-500/20 select-none">
              S
            </div>
            <div>
              <h1 className="text-lg font-bold text-zinc-900 tracking-tight leading-none">SNAP</h1>
              <p className="text-[10px] font-medium text-emerald-600 uppercase tracking-wider mt-0.5">Smart Accessibility</p>
            </div>
          </div>
        ...
        <div className="bg-zinc-100/80 rounded-xl p-1 flex gap-1">
          ...
          <button
            onClick={() => {
              ...
              handleOverlay(); // Launch the overlay immediately
              window.close(); // Close the popup since we are moving to overlay
            }}
            className={`flex-1 h-9 rounded-lg text-xs font-bold transition-all duration-200 ${
              mode === 'toolbar'
                ? 'bg-white text-emerald-600 shadow-sm'
    ...
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-sm overflow-hidden border border-zinc-200">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide p-4 space-y-4 bg-zinc-50/30">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center p-6 select-none animate-in fade-in duration-500">
            <div className="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center mb-4 ring-1 ring-emerald-100">
              <MessageSquare size={32} className="text-emerald-500/80" strokeWidth={1.5} />
            </div>
      ...
      <div className="p-3 bg-white border-t border-zinc-100">
        <div className="flex gap-2 items-center bg-zinc-50 rounded-xl p-1.5 border border-zinc-200 focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all">
          <button className="p-2 rounded-lg text-zinc-400 hover:text-emerald-600 hover:bg-emerald-50 transition-all">
            <Mic size={18} />
          </button>
          <input
            value={input}
          ...
          <button
            onClick={doSend}
            disabled={!input.trim()}
            className="p-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 disabled:opacity-50 disabled:hover:bg-emerald-500 transition-all shadow-sm active:scale-95"
          >
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 space-y-5">
        ...
        <div className="space-y-3">
          <button
            ...
            className="relative w-full h-14 group bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl p-[1px] shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 transition-all active:scale-[0.98]"
          >
             ...
             <div className="h-full w-full bg-white/0 flex items-center px-4 rounded-xl gap-3">
               ...
               <div className="flex flex-col items-start">
                 ...
                 <span className="text-emerald-100 text-[10px] font-medium">Rewrite complex text simply</span>
               </div>
             </div>
          </button>
